...
* A **_state_** is an identity at a point in time.
* Updating an identity doesn't destroy old values.
* Clojure's reference model separates identities from values.
* Almost everything in Clojure is a value.
* For identities, Clojure has 4 types:
...
* These will cause the thread to block until all actions sent from current thread or agent have completed.
...
* `send-off` is a variant of `send` for actions that expect to block, as a file write with `spit` might do.
...
* The call to `.start` returns `nil`, and the value of `foo` is printed from the new thread.

* Bindings have _dynamic_ scope
* Visible everywhere during a thread's execution, until the thread exits t scope where the binding began.
